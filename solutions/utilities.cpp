...
std::string padStringWithChar(const std::string &word, char c,
                              size_t sizeToFixTo, bool side) {
    using std::string;
...
std::string base64ToHex(const std::string &base64String) {
    ...
    for (auto symbol : base64String) {
        ...
        binString +=
            padStringWithChar(decimalToBin(symbolIndex), '0', 6, false);
    }
...
double evaluateText(const std::string &text) {
    ...
    map<char, double> englishLetterFrequency = {
        ...
        {'y', 2.11},  {'w', 2.09}, {'g', 2.02}, {'p', 1.82}, {'b', 1.49},
        {'v', 1.11},  {'k', 0.69}, {'x', 0.17}, {'q', 0.11}, {'j', 0.10},
        {'z', 0.07},  {' ', 13.29}};

    // create a map so that we don't have to iterate over the text every time we
    // want to count a letter
    ...
    // counts the frequency of every letter in the text if its in the frequency
    // table
    for (auto c : text) {
    ...
    // accumulates the error between the text letter frequency and the english
    // letter frequency
    double sum = 0;
    for (auto [letter, count] : textLettersCount) {
        double textCurrentLetterFrequency = count / text.length();
        double error = std::abs(textCurrentLetterFrequency -
                                englishLetterFrequency[letter]);
        sum += error;
    }
...
CryptoText<unsigned char> decrypt_message(std::string binString) {
    ...
    for (int keyCandidate = 0; keyCandidate < 256; keyCandidate++) {
        ...
        textCandidates[keyCandidate] = {textCandidate,
                                        (unsigned char)keyCandidate,
                                        evaluateText(textCandidate)};
    }

    // get the smallest score between the candidates
    // (smaller error accumulation)
    CryptoText bestCandidate =
        *std::min_element(textCandidates.begin(), textCandidates.end(),
                          [](const CryptoText<unsigned char> &a,
                             const CryptoText<unsigned char> &b) {
...
transposeMatrix(const std::vector<std::vector<std::string>> &matrix) {
    using std::string;
    using std::vector;
